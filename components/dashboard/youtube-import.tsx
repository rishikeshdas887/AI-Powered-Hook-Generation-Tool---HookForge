...
import { Youtube, Loader2, ChevronRight, AlertCircle, CheckCircle, RefreshCw } from 'lucide-react';
...
export function YouTubeImport({ session, onSelectTopic }: YouTubeImportProps) {
  ...
  const handleOpen = (isOpen: boolean) => {
    setOpen(isOpen);
    if (isOpen && videos.length === 0 && !loading) {
      fetchVideos();
    }
  };
    ...
    return (
      <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-border bg-card/50">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-muted-foreground">Sign in with Google to import your videos</p>
        </div>
